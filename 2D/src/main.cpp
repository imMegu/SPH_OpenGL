...
void renderGUI() {
  ...
  ImGui::SliderFloat("Density", &targetDensity, 0.01f, 50000.0f);
...
int main() {
  ...
  uint32_t updateShader =
      CompileShader(updateShaderSource, GL_COMPUTE_SHADER, "UPDATE");
  ...
  uint32_t partitionShader = CompileShader("/src/shader/partition.compute",
                                           GL_COMPUTE_SHADER, "PARTITION");
  ...
  uint32_t sortShader =
      CompileShader("/src/shader/sort.compute", GL_COMPUTE_SHADER, "SORT");
  ...
  uint32_t offsetsShader = CompileShader("/src/shader/offsets.compute",
                                         GL_COMPUTE_SHADER, "OFFSETS");
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, NUM_PARTICLES * sizeof(glm::vec2),
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, NUM_PARTICLES * sizeof(glm::vec2),
               &predictedPositions[0], GL_STATIC_DRAW);
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, NUM_PARTICLES * sizeof(uint32_t),
               &particleIndices[0], GL_DYNAMIC_DRAW);
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER,
               TOTAL_GRID_CELL_COUNT * sizeof(uint32_t), &cellOffsets[0],
               GL_DYNAMIC_DRAW);
...
void runSimulationFrame(GLuint offbuf) {
  if (running) {
    ...
    // 4. Sort particles by cell indices (simplified bitonic sort or
    // countingSort)
...
void mouse_callback(GLFWwindow *window, double xposIn, double yposIn) {
  ...
  float normalizedY =
      1.0 - ypos / SCR_HEIGHT; // Normalize y and flip vertically
