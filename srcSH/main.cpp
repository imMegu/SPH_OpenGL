...
void runSimulationFrame(GLuint offbuf)
{
    if (running)
    {
        ...
        // 4. Sort particles by cell indices (simplified bitonic sort)
        ...
        setFloat(computeProgram[2], "SpikyPow2DerivativeScalingFactor", 12 / (pow(smoothingRadius, 4) * M_PI));
...
glm::vec3 genRandomVector2d()
{
    ...
    std::uniform_real_distribution<float> dist_x(((botX + 0.35f * (topX - botX))), (botX + 0.65f * (topX - botX)));
    std::uniform_real_distribution<float> dist_y(((botY + 0.10f * (topY - botY))), (botY + 0.90f * (topY - botY)));
